...
# ╔═╡ 5a4877c5-6d08-4621-925d-0aaa39137237
a_range = -4:0.25:4

# ╔═╡ bf27e1f3-61b1-4769-9a2c-3d3937284a88
x0_range = -1:0.2:1
...
|       | Initial conditions |
|:-----:|:-----------------------------:|
| x₁(0) | $(@bind x₁0 Slider(x0_range; default=1)) |
| x₂(0) | $(@bind x₂0 Slider(x0_range; default=1)) |
"""
...
# ╔═╡ f8d4113e-b68f-44d4-92c3-a9f61a192f28
t_max = 10
...
begin
	fig_traj, ax_traj = scatter(Point2f(sol.u[1]), marker=:star5, markersize=40)
	...
	xlim_traj = ceil(maximum(
		hcat(map(e -> e[1], sol.u), map(e -> e[2], sol.u))
	)*10)/10
	#limits!(ax_traj, -xlim_traj, xlim_traj, -xlim_traj, xlim_traj)
	trajectory = Observable(Point2f[])
	scatter!(ax_traj, trajectory)
	CairoMakie.Makie.Record(fig_traj, 1:length(sol.t)) do k
		push!(trajectory[], sol.u[k])
		ax_traj.title = "t = $(sol.t[k])"
	end
end
...
L"""
\lambda_1 = %$(complex_to_string(λ1)),\quad
\lambda_2 = %$(complex_to_string(λ2))
"""

# ╔═╡ 452529c3-9194-4c8b-b2ca-9fdae3781ef7
function matrix_to_string(A)
	rows = map(row -> join(map(complex_to_string, row), "&"), eachrow(A))
	join([
		"\\left[\\begin{matrix}",
		join(rows, " \\\\ "),
		"\\end{matrix}\\right]"
...
L"x(0) = %$(matrix_to_string([x₁0; x₂0]))"

# ╔═╡ e704ecd0-53c3-4e32-9de9-20684f272e49
L"Q = %$(matrix_to_string(Q))"
...
|   A    | 1         | 2         |
|:-----:|:-------------:|:-------------:|
| 1 | $(@bind a₁₁ Slider(a_range; default=0)) | $(@bind a₁₂ Slider(a_range; default=1)) |
| 2 | $(@bind a₂₁ Slider(a_range; default=-1)) | $(@bind a₂₂ Slider(a_range; default=0)) |
"""
  ╠═╡ =#

# ╔═╡ 8570b761-ae29-4c1e-ad67-cd565e9d72bb
begin
	a₁₁ = 1.0
	a₁₂ = 1.0
	a₂₁ = 4.0
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "0964b4693bbe9858d1c3cd42e538586da4c54764"
